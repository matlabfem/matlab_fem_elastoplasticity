...
%
  ...
  switch(elem_type)
    case 'P1'
        [COORD,ELEM,SURF,NEUMANN,Q]=mesh_P1(level,size_xy,size_hole);
        fprintf('P1 elements: \n')
    case 'P2'
        [COORD,ELEM,SURF,NEUMANN,Q]=mesh_P2(level,size_xy,size_hole);
        fprintf('P2 elements: \n')
    case 'Q1'
        [COORD,ELEM,SURF,NEUMANN,Q]=mesh_Q1(level,size_xy,size_hole);
        fprintf('Q1 elements: \n')
    case 'Q2'
        [COORD,ELEM,SURF,NEUMANN,Q]=mesh_Q2(level,size_xy,size_hole);
        fprintf('Q2 elements: \n')
    otherwise
...
%
  ...
  Hard_old = zeros(4,n_int); % hardening tensors at integration points
  ...
  for i=2:n_step            
    ...
    if  mod(i-1,10)==0           %(mod(i,ceil((n_step)/6))==0)
        ...
        %draw_quantity(COORD,ELEM,10*U,U_total,elem_type,size_xy,size_hole)
               
        % hardening
        Hard_node = transformation(sqrt(sum(Hard.^2)),ELEM,WEIGHT); 
        draw_quantity(COORD,ELEM,10*U,zeros(size(Hard_node))+Hard_node,elem_type,size_xy,size_hole) 
