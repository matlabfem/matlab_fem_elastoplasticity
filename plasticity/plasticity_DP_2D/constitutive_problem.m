...
%
% Input data:
%  E_new - current strain tensor, size(E_new)=(3, n_int)
...
%   S_tr - trial stress tensors, size(E_tr)=(4,n_int)
%   dev_E - deviatoric part of the trial strain, size(SD_tr)=(4,n_int)
%   norm_E - norm of dev_E, size(norm_SD)=(1,n_int)
%
  E4=[E; zeros(1,n_int)];
  E_tr = E4-Ep_prev ;                   % trial strain                           
...
%
  ...
  N_hat=dev_E(:,IND_s)./repmat(norm_E(IND_s),4,1);
  M_hat=repmat(sqrt(2)*shear(IND_s),4,1).*N_hat+IOTA*(bulk(IND_s).*eta(IND_s));
  S(:,IND_s)=S(:,IND_s)-repmat(lambda_s,4,1).*M_hat;
  ...
  MM_hat=repmat(M_hat(1:3,:),3,1).*kron(M_hat(1:3,:),ones(3,1));
  DS(:,IND_s)=DS(:,IND_s)-...
      repmat(2*sqrt(2)*(shear(IND_s).^2).*lambda_s./rho_tr(IND_s),9,1).*(ID-NN_hat)-...
      MM_hat./repmat(denom_s(IND_s),9,1);
  DS(:,IND_a)=zeros(9,n_apex);
...
%
  ...
  if nargout<=3          
     ...
     % plastic strain
     Ep=Ep_prev;
     Ep(:,IND_s)=Ep(:,IND_s)+([1;1;2;1]*lambda_s).*(N_hat/sqrt(2)+IOTA*(eta(IND_s)/3));
     Ep(:,IND_a)=E4(:,IND_a)-IOTA*(c(IND_a)./(3*bulk(IND_a).*eta(IND_a)));     
  end
