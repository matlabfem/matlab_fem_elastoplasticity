...
function [coord,elem,surf,neumann,dirichlet,Q]=...
                    mesh_P1(level,size_xy,size_z,size_hole)
...
%    neumann   - array containing numbers of nodes defining each surface
%                element, size(neumann)=(3,n_e_s). The surface is the following
%                side of the body: (0,size_xy) x size_xy x (0,size_z), where
%                the nonhomogeneous Neumann boundary condition is considered.
%    dirichlet - array indicating the nodes where the nonhomogeneous Dirichlet
%                boundary condition is considered, size(dirichlet)=(3,n_n)
...
%
  ...
  surf1=reshape(aux_surf,3,2*(N_x*N_y-N1_x*N1_y));       
  ...
  aux_surf=[C_s(V1_s)'; C_s(V2_s)'; C_s(V4_s)';
            C_s(V3_s)'; C_s(V4_s)'; C_s(V2_s)' ];
  surf2=reshape(aux_surf,3,2*(N_x*N_y-N1_x*N1_y));       
  ...
  aux_surf=[C_s(V1_s)'; C_s(V2_s)'; C_s(V4_s)';
            C_s(V3_s)'; C_s(V4_s)'; C_s(V2_s)' ];
  surf6=reshape(aux_surf,3,2*N2_y*N_z);     
  ...
  aux_surf=[C_s(V4_s)'; C_s(V1_s)'; C_s(V3_s)';
            C_s(V2_s)'; C_s(V3_s)'; C_s(V1_s)' ];
  surf7=reshape(aux_surf,3,2*N1_x*N_z);       
  ...
  aux_surf=[C_s(V1_s)'; C_s(V2_s)'; C_s(V4_s)';
            C_s(V3_s)'; C_s(V4_s)'; C_s(V2_s)' ];
  surf8=reshape(aux_surf,3,2*N1_y*N_z);  
