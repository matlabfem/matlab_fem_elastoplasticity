...
%
  ...
  switch(elem_type)
    case 'P1'; [COORD,ELEM,SURF,NEUMANN,DIRICHLET,Q]=mesh_P1(level,size_xy,size_z,size_hole);
    case 'P2'; [COORD,ELEM,SURF,NEUMANN,DIRICHLET,Q]=mesh_P2(level,size_xy,size_z,size_hole);
    case 'Q1'; [COORD,ELEM,SURF,NEUMANN,DIRICHLET,Q]=mesh_Q1(level,size_xy,size_z,size_hole);
    case 'Q2'; [COORD,ELEM,SURF,NEUMANN,DIRICHLET,Q]=mesh_Q2(level,size_xy,size_z,size_hole);
    otherwise
...
% Assembling of the stiffness matrix
  ...
  tic; 
  [K,WEIGHT]=elastic_stiffness_matrix(ELEM,COORD,shear,bulk,DHatP1,DHatP2,DHatP3,WF);  
  assembly_time=toc; 
  rows=size(K,1);
...
%  
  ...
  f_V_int=volume_force'*ones(1,n_int); % size(f_V_int)=(3,n_int)
...
%
% Ud - 3*n_n array representing constant displacement in y-direction 
...
%
  if draw
    ...
    draw_displacement(COORD,SURF,U,U_total,elem_type,size_xy,size_z,size_hole)
